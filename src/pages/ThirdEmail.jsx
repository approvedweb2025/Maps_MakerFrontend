...
const ThirdEmail = () => {
  // State management bilkul SecondEmail jaisa kar diya gaya hai
  ...
  // Single image preview ke liye states
  ...
  const fetchPhotos = async () => {
    try {
      ...
      // Photos ko metadata (year, district) ke saath enrich karna
      ...
      // Photos ko group karne ka logic (SecondEmail se liya gaya)
  ...
  // "View All" modal kholne ka function
  ...
  // Single image preview kholne ka function
  ...
  // Reusable component jo images ka grid show karega
    ...
    <div className="px-4 py-8 max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        Object.entries(photosByYear)
          .sort((a, b) => b[0] - a[0]) // Saal ko descending order mein sort karna
          .map(([year, yearPhotos]) => (
            <ImageGrid key={year} title={year} images={yearPhotos} />
          ))
      ) : (
        Object.entries(photosByDistrict)
          .sort(([a], [b]) => a.localeCompare(b)) // District ko alphabetically sort karna
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70" onClick={() => setPreviewImage(null)}>
          <div
            className={`relative flex items-center justify-center ${
              isFullscreen ? "w-screen h-screen" : "w-[90vw] h-[90vh] max-w-4xl max-h-4xl"
            }`}
