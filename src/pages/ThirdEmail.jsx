...
const ThirdEmail = () => {
  ...
  const fetchPhotos = async () => {
    try {
      setLoading(true);
      console.log("🔄 ThirdEmail: Fetching photos...");
      ...
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_BASE_URL}/photos/getImages/muhammadjig8@gmail.com`
        );
        console.log("✅ ThirdEmail API response:", res.data);
        rawPhotos = res.data.photos || [];
        console.log(`📊 ThirdEmail: Found ${rawPhotos.length} photos from specific endpoint`);
      } catch (endpointErr) {
        ...
        try {
          ...
          if (fallbackRes.status === 200) {
            rawPhotos = (fallbackRes.data.photos || []).filter(p => p.uploadedBy === 'muhammadjig8@gmail.com');
            console.log(`📊 ThirdEmail: Found ${rawPhotos.length} photos from fallback`);
          }
        } catch (fallbackErr) {
      ...
      console.log(`🎯 ThirdEmail: Processed ${enrichedPhotos.length} photos`);
      
      // Debug: Log first few photo URLs
      if (enrichedPhotos.length > 0) {
        console.log("🔍 ThirdEmail: Sample photo URLs:", enrichedPhotos.slice(0, 3).map(p => ({ 
          name: p.name, 
          url: p.url, 
        ...
        {images.slice(0, 6).map((photo) => (
          <div key={photo.fileId} className="relative">
            <img
              ...
              onError={(e) => {
                console.log(`❌ ThirdEmail: Image failed to load: ${photo.url}`);
                // Fallback to Google Drive if Cloudinary fails
                const fallbackId = photo.driveFileId || photo.fileId;
                if (fallbackId) {
                  console.log(`🔄 ThirdEmail: Trying fallback URL for ${photo.name}`);
                  e.currentTarget.src = `https://drive.google.com/uc?export=view&id=${fallbackId}`;
                } else {
                  console.log(`❌ ThirdEmail: No fallback available for ${photo.name}`);
                }
              }}
            />
            {!photo.url && (
  ...
  return (
    <div className="px-4 py-8 max-w-7xl mx-auto">
      <h1 className="text-3xl font-bold text-center mb-6 text-gray-800 dark:text-white">
        Third Email Photos (muhammadjig8@gmail.com)
      </h1>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {modalPhotos.map((photo) => (
                <img
                  ...
                  onError={(e) => {
                    console.log(`❌ ThirdEmail Modal: Image failed to load: ${photo.url}`);
                    // Fallback to Google Drive if Cloudinary fails
                    const fallbackId = photo.driveFileId || photo.fileId;
                    if (fallbackId) {
                      console.log(`🔄 ThirdEmail Modal: Trying fallback URL for ${photo.name}`);
                      e.currentTarget.src = `https://drive.google.com/uc?export=view&id=${fallbackId}`;
                    } else {
                      console.log(`❌ ThirdEmail Modal: No fallback available for ${photo.name}`);
                    }
                  }}
                />
              ))}
